...
```bash
adoptrank collect --queries seed_queries.txt --output data/snapshots/latest.jsonl
adoptrank analyze-code --snapshots data/snapshots/latest.jsonl --output data/snapshots/code.jsonl --limit 20
adoptrank dataset --snapshots data/snapshots/code.jsonl --output data/training/pairs.jsonl
adoptrank train --dataset data/training/pairs.jsonl --snapshots data/snapshots/code.jsonl --model-dir artifacts/current
adoptrank serve --model-dir artifacts/current
```
