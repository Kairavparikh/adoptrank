...
M1 is implemented as a local end-to-end vertical slice and the web client is deployed. The repository contains real GitHub/PyPI collectors, PostgreSQL/pgvector migrations, a FastAPI service, PyTorch pairwise ranker, tests, and a Vercel-hosted UI. M2 is in progress: commit-pinned code extraction and code-derived model features are implemented, followed by live indexing, retraining, and UI evidence rendering.
